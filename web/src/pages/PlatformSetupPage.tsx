    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-950 to-slate-900">
      <div className="mx-auto max-w-2xl px-4 py-10">
        <header className="mb-8 text-center">
          ...
          <h1 className="mt-2 text-3xl font-semibold text-white">Configure your control plane</h1>
        ...
        <div className="rounded-2xl border border-slate-800 bg-slate-900/90 p-8 shadow-xl">
          {error && (
            <p className="mb-4 rounded-lg bg-red-950/50 px-4 py-3 text-sm text-red-300">{error}</p>
          )}
          ...
          {step === "welcome" && (
            <div className="space-y-4 text-slate-300">
              ...
              <ul className="list-inside list-disc space-y-2 text-sm text-slate-400">
                <li>Create your first <strong className="text-slate-300">organization</strong> (tenant boundary)</li>
                <li>Optionally add a <strong className="text-slate-300">project</strong> for workloads</li>
                <li>Review <strong className="text-slate-300">security</strong> recommendations before going live</li>
              </ul>
              <p className="text-sm text-slate-500">
                You can add agents, networks, and more from the console after setup. Use{" "}
                <strong className="text-slate-400">New</strong> in the header anytime to create
          ...
          {step === "security" && (
            <div className="space-y-4 text-sm text-slate-300">
              ...
              <ul className="list-inside list-disc space-y-2 text-slate-400">
                <li>
                  Set strong values in <code className="text-emerald-400">.env</code> for{" "}
                  <code className="text-slate-300">BOOTSTRAP_ADMIN_PASSWORD</code> and{" "}
                  <code className="text-slate-300">JWT_SECRET</code> (32+ characters)
                </li>
                <li>Align service tokens across registry, inventory, and projects</li>
                <li>Enable OIDC / Keycloak when you are ready for SSO (<code className="text-slate-300">docker compose --profile sso up</code>)</li>
              </ul>
            </div>
          )}
          ...
          {step === "done" && (
            <div className="space-y-3 text-center text-slate-300">
              <p className="text-lg font-medium text-emerald-400">Setup complete</p>
              <p className="text-sm text-slate-400">
                {orgName ? (
                  <>
                    Organization <strong className="text-white">{orgName}</strong> is ready.
                  </>
                ) : (
              ...
              <p className="text-xs text-slate-500">
                Use <strong className="text-slate-400">New</strong> / <strong className="text-slate-400">Delete</strong>{" "}
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {stepIndex > 0 && step !== "done" && (
              <button
                ...
                onClick={goBack}
                disabled={busy}
                className="min-h-11 rounded-lg border border-slate-600 px-5 text-sm hover:bg-slate-800 disabled:opacity-50"
              >
                Back
              </button>
            )}
            ...
            {step === "project" && (
              <>
                <button
                  ...
                  disabled={busy}
                  className="min-h-11 rounded-lg border border-slate-600 px-5 text-sm hover:bg-slate-800"
                >
                  Skip for now
                </button>
                <button
