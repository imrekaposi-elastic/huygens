...
function ProviderDetail({ providerId }: { providerId: string }) {
  ...
  if (isLoading || !data) return <p className="text-slate-500">Loading tree…</p>;
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <span
          className={`rounded px-2 py-0.5 text-xs ${
            data.operational ? "bg-emerald-950 text-emerald-300" : "bg-slate-800 text-slate-400"
          }`}
        >
          {data.operational ? "Provider operational" : "Not operational"}
        </span>
        <span className="text-xs text-slate-500">{data.total_agents} enrolled agent(s)</span>
      </div>
      <RegionTreePanel nodes={data.region_tree} />
      <form
        className="mt-4 flex flex-col gap-3 rounded-lg border border-slate-800 bg-slate-950/50 p-4 md:flex-row md:flex-wrap md:items-end"
        onSubmit={(e) => {
          e.preventDefault();
          addRegion.mutate();
        }}
      >
        <label className="text-sm md:w-full">
...
export function InfrastructureProvidersPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">Infrastructure providers</h1>
      ...
      <div className="space-y-3">
        {providers?.map((p) => (
          <section key={p.id} className="rounded-xl border border-slate-800 bg-slate-900 p-5">
            <button
              type="button"
            ...
            >
              <div>
                ...
                <span className="ml-2 font-mono text-sm text-slate-500">{p.slug}</span>
              </div>
              <span
                className={`rounded px-2 py-0.5 text-xs ${
                  p.operational ? "bg-emerald-950 text-emerald-300" : "bg-slate-800 text-slate-400"
                }`}
              >
