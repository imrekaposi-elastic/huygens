...
import { AgentTechIcon, PageTitle, ShieldIcon } from "@/components/icons/NavIcons";
...
const FABRIC_TAB = { to: "/admin/fabric" as const, label: "Fabric" } as const;
  ...
  return (
    <div className="space-y-6">
      ...
      <nav className="flex flex-wrap gap-2 border-b border-slate-200 dark:border-slate-800 pb-2">
        {[...(platformAdmin ? [FABRIC_TAB, ...TABS_BASE] : [...TABS_BASE])].map((tab) => {
          ...
          return (
            ...
            >
              {tab.to === FABRIC_TAB.to && (
                <AgentTechIcon className="size-4 shrink-0 opacity-80" />
              )}
              {tab.label}
            </Link>
          );
