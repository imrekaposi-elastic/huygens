...
export function AgentRegisterDialog({ open, organizationId, onClose, onRegistered }: Props) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!infrastructureProviderId || !regionId || !agentTechnologyId) {
      setError("Select infrastructure provider, region, and agent technology.");
      return;
    }
    setBusy(true);
    setError(null);
    ...
    >
      <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-xl border border-slate-700 bg-slate-900 p-6 shadow-xl">
        ...
        <p className="mt-1 text-sm text-slate-400">
          Enroll at a region node. Coverage propagates to parent regions and the infrastructure
          provider.
        </p>
        ...
        {issuedToken ? (
          <div className="mt-4 space-y-3">
            ...
            <p className="text-sm text-emerald-400">Agent registered. Save this token:</p>
            <pre className="overflow-x-auto rounded-lg bg-slate-950 p-3 text-xs text-amber-200">
              {issuedToken}
            </pre>
            <button
              type="button"
          ...
          <form onSubmit={submit} className="mt-4 space-y-3">
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button type="button" onClick={onClose} className="min-h-11 rounded-lg border border-slate-600 px-4 text-sm">
                Cancel
              </button>
              <button
                type="submit"
